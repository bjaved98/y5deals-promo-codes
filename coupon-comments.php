...
if ( $comments ) {
    $args = array(
        ...
        'type'          => 'all',
        'reply_text'    => esc_html__( 'Reply', 'wp-coupon-mate' ),
        'avatar_size'   => 32,
    );
    ...
    if (  $c_paged < $total_page ) {
        ...
        <div class="load-more wpb_content_element">
            <a class="load-more-btn ui button btn btn_primary btn_small" data-c-paged="<?php echo ( $c_paged + 1 ); ?>" data-loading-text="<?php esc_attr_e('Loading...', 'wp-coupon-mate'); ?>" href="#"><?php esc_html_e('Load More Comments', 'wp-coupon-mate'); ?>
    ...
    if ( $c_paged <= 1 ) {
        ?>
        <h4 class="cm-no-comments"><?php esc_html_e( 'No comment, be the first.', 'wp-coupon-mate' ) ?></h4>
        <?php
    }
