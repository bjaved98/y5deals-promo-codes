                        ...
                        echo '<div class="post-meta-data">';
                            ...
                            if ( is_object( $authordata ) ) {
                                echo '<span class="author-name">';
                                    printf(
                                        esc_html__( 'Posts by %s' , 'wp-coupon-mate' ),
                                        '<a href="'.esc_url( get_author_posts_url( $authordata->ID, $authordata->user_nicename ) ).'" title="'.esc_attr( sprintf( esc_html__( 'Posts by %s', 'wp-coupon-mate' ), get_the_author() ) ).'" rel="author">'. get_the_author().'</a>'
                                    );
                                echo '</span>';
                            }
                            echo '<span class="comment-number">';
                                comments_number(
                                    esc_html__( '0 Comments', 'wp-coupon-mate' ),
                                    esc_html__( '1 Comment', 'wp-coupon-mate' ),
                                    esc_html__( '% Comments', 'wp-coupon-mate' )
                                );
                            echo '</span>';
                            echo '</p>';
                        echo '</div>';
